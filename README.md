...
**주제**: 매일 밤, 마음의 방향을 찾는 질문 — 위로와 성찰의 카드뉴스 (`config.yaml`에서 변경 가능)
...
3. **아무 페이지나 만들어서 연결**: Notion에서 빈 페이지를 하나 만듭니다 (데이터베이스일 필요 없음 — 그냥 일반 페이지). 우측 상단 `⋯` 메뉴 → **"연결 추가(Add connections)"** → 1번에서 만든 통합을 선택합니다. (이 단계를 빠뜨리면 API가 403 오류를 반환합니다.)
4. **자동 설정 실행**: Actions 탭 → "Setup & Clean Notion Database" → "Run workflow". `scripts/setup_notion_db.py`가:
   - 통합에 이미 연결된 데이터베이스가 있으면 그것을 사용하고,
   - 없으면 3번에서 연결한 페이지 안에 **"카드뉴스 콘텐츠"라는 데이터베이스를 필요한 속성을 모두 갖춰서 자동으로 새로 만들고** (제목/날짜/카테고리/상태/post_id/소재/슬라이드수/해시태그/GitHub Pages 링크/발행일시/media_id/좋아요/댓글/저장/공유/도달/참여점수),
   - 찾았거나 새로 만든 데이터베이스의 id를 `config.yaml`의 `notion.database_id`에 자동으로 기록해 커밋합니다.

   즉 데이터베이스나 속성을 직접 만들 필요 없이, 빈 페이지 하나 연결하고 이 워크플로 한 번 실행하면 끝입니다. 몇 번을 다시 실행해도 안전합니다 (이미 있는 데이터베이스/속성은 건드리지 않음).
   같은 워크플로가 `content/history.json`에 더 이상 없는(주제 전환 등으로 정리된) 옛 초안의 Notion 페이지도 자동으로 보관 처리(archive)합니다.
...
- **콘텐츠 성격에 대한 주의**: 이 계정은 심리 상담이나 치료가 아닙니다. 진단·처방하는 어투, 특정 질환/증상 언급, 전문 치료를 대체할 수 있다는 인상을 주지 않도록 프롬프트를 설계했습니다. 그래도 정기적으로 실제 게시물을 직접 검수하는 것을 권장합니다.
- **토큰 만료**: 장기 토큰(60일) 방식이면 만료 전 갱신 필요. System User 토큰 사용을 권장합니다 (2-5 참고).
- **게시 빈도**: Instagram Graph API는 24시간당 최대 25건 게시 제한이 있습니다. 현재 이틀에 1건(안정화 후 매일 1건으로 전환 예정) 스케줄이면 문제없습니다.
